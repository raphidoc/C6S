...
//        }
    ...
    const char * pipeIn = "/tmp/6s_pipe_in";
    const char * pipeOut = "/tmp/6s_pipe_out";
    mkfifo(pipeIn, 0666);
    mkfifo(pipeOut, 0666);
    ...
    if (pid == 0) { // Child process
        freopen(pipeOut, "r", stdout); // Redirect stdin to named pipe

        char command[1000];
        sprintf(command, "cat %s | /bin/sh", pipeIn);
        execlp("/bin/sh", "/bin/sh", "ll", command, nullptr); // Execute shell

        perror("execlp");
        exit(EXIT_FAILURE);
    } else if (pid > 0) { // Parent process
        std::ofstream pipeStream(pipeIn); // Open named pipe for writing

        for (int i = 0; i < commands.size(); i++) {
            pipeStream << commands[i] << "\n"; // Write command to named pipe
            pipeStream.flush();

//            char buffer[3000];
//            std::string jsonOutput;
//            while (read(pipeOut, buffer, sizeof(buffer)) != 0) {
//                jsonOutput += buffer;
//            }
        }
    } else {
